// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.math.controller.PIDController;
import edu.wpi.first.wpilibj2.command.CommandBase;
import frc.robot.subsystems.Arm;
import static frc.robot.Constants.ArmConstants.*;

public class ChangeArmManually extends CommandBase {

  private Arm m_Arm;

  private int direction;

  /** Creates a new ChangeArmAngle. */
  public ChangeArmManually(Arm subsystem, int dir) {

    m_Arm = subsystem;
    
    direction = dir;

    // Use addRequirements() here to declare subsystem dependencies.
    addRequirements();
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    m_Arm.setAngleMotorSpeed((direction > 0) ? kAngleMotorManualSpeed : -kAngleMotorManualSpeed);
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    m_Arm.setAngleMotorSpeed(0);

  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    if (m_Arm.getMinLimitSwitch() && m_Arm.getAngleMotorSpeed() < 0) {
      return true;
    }
    if (m_Arm.getMaxLimitSwitch() && m_Arm.getAngleMotorSpeed() > 0) {
      return true;
    }
    // stop the motor after reaching the desired angle
    // return false;
    return false;
  }
}
